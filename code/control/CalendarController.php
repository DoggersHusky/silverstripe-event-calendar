...
class CalendarController extends PageController {
	...
	public function ics(HTTPRequest $r) {
		...
		if(is_numeric($id) && $oid) {
			...
			$LANGUAGE = Calendar::config()->get('language');
			$CALSCALE = "GREGORIAN";
			$parts = explode('-',$oid);
	...
	public function DateHeader() {
		switch($this->view) {
			case "day":
				return CalendarUtil::localize($this->startDate->get(), null, CalendarUtil::ONE_DAY_HEADER, Calendar::config()->get('language'));
			break;

			case "month":
				return CalendarUtil::localize($this->startDate->get(), null, CalendarUtil::MONTH_HEADER, Calendar::config()->get('language'));
			break;

			case "year":
				return CalendarUtil::localize($this->startDate->get(), null, CalendarUtil::YEAR_HEADER, Calendar::config()->get('language'));
			break;
